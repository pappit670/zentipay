...
description: Claude Design template-first workflow for authoring HyperFrames video compositions (plain HTML + CSS + paused GSAP timeline rendered to MP4 by the hyperframes CLI). Use when drafting a HyperFrames composition skeleton, scenes, palette/type, entrance + mid-scene animations, and shader transitions for the user to render locally.
---
